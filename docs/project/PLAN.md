...
| M3 | R10 全文学习视图分批研读、本地离线模型适配、R11–R12、R15 完整能力与真机走查 | 全部 P0 与 V0.1 范围回归，离线模型适配调度，全文分批研读，真机物理手写走查闭环 | **IN_PROGRESS (实施中)** |
...
- **待收敛 (M3 实施期)**：
  - R10 全文学习视图的长文档分批抽取引擎与树状概念大纲聚合；
  - 本地离线模型框架（MLX / llama.cpp / CoreML）集成与统一抽象调度；
  - 实体 iPad 设备与 Apple Pencil 硬件真机走查（压感、手掌防误触、渲染延迟与外部生产公网稳定性）。
